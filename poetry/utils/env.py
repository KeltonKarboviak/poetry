...
class EnvManager(object):
    ...
    def activate(self, python, io, cwd=None):  # type: (str, IO, Optional[Path]) -> Env
        if cwd is None:
            cwd = Path.cwd()
        ...
        if not venv.exists() or venv.exists() and create:
            self.create_venv(io, cwd=cwd, executable=python, force=create)

        # Activate
        envs[cwd.name] = {"minor": minor, "patch": patch}
        envs_file.write(envs)

        return self.get(cwd=cwd, reload=True)

    def deactivate(self, io, cwd=None):  # type: (IO, Optional[Path]) -> None
        if cwd is None:
            cwd = Path.cwd()
    ...
    def get(self, reload=False, cwd=None):  # type: (bool, Path) -> Env
    ...
    def create_venv(
        self, io, name=None, cwd=None, executable=None, force=False
    ):  # type: (IO, bool, Path, str, bool) -> Env
        if self._env is not None:
            return self._env

        env = self.get(cwd=cwd, reload=True)
